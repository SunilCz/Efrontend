...
const Cart = () => {
  ...
  const handlePayment = () => {
    ...
    const config = {
      ...
      amount: subTotal * 100, // Convert the amount to the smallest currency unit (e.g., paisa for NPR),
      eventHandler: {
        onSuccess(payload) {
          // Payment successful
          console.log(payload);
        ...
        onClose() {
          ...
          setLoading(false);
          // Handle the event if the user closes the payment window without completing the payment
        },
      },
    };
